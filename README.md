...
Build a landing page + dashboard web app called "Axiom" — a privacy-first

AI trading agent dApp on Midnight. Dark, minimal, high-trust fintech

aesthetic (Linear x crypto terminal — not a generic SaaS template). Deep

charcoal background, one sharp accent color (electric violet or teal),

generous whitespace, monospace accents for numbers/hashes/addresses,

subtle grid/glow details that read "cryptographic" without being kitschy.

Structure the React code to match this file layout exactly:

- src/components/WalletConnect.tsx — wallet connect button + connected state

- src/components/StrategyBuilder.tsx — the core privacy feature, see below

- src/components/Layout.tsx — persistent sidebar shell wrapping all pages

- src/App.tsx — routes

- src/main.tsx — entry point

PAGES (all live inside the Layout.tsx sidebar shell once wallet is connected):

1. Landing page (pre-connect, outside Layout)

   - Hero: "Trade on your own rules. Provably." Subhead: describe a

     strategy once, every trade after is proven to follow it, nothing

     about the strategy is ever revealed.

   - 3-step "how it works": Describe strategy → Commit hash on-chain →

     Every trade proven, nothing disclosed.

   - WalletConnect CTA.

2. Home / Overview

   - Active strategy cards, aggregate P&L stat, recent trade status feed

     (executed/rejected badges), small market ticker strip.

3. Market Insights

   - Price chart area, AI-generated insight cards per asset (tone +

     short reasoning).

4. Strategy Builder (StrategyBuilder.tsx — this is the core feature,

   make it the most polished screen in the app)

   - Large natural-language input: "only buy ADA, max 20% position, 8%

     stop-loss, run for 30 days"

   - "Review & Confirm" card: parsed fields (asset, max position %,

     stop-loss %, timeline) shown as editable chips before commit

   - "Commit Strategy" button, with a loading state that reads like a

     proof is being generated (not a generic spinner) and a success

     state showing the resulting commitment hash (monospace, truncated

     with copy button) — make this moment feel like the privacy

     guarantee is visibly happening, not hidden behind a plain form

     submit.

5. Portfolio

   - Big live P&L number (green/red), portfolio value line chart,

     position list (entry vs current price), SIP-style insight line

     ("up 12% over 18 days").

6. Trade History

   - Table: timestamp, status pill (executed/rejected) only — no

     strategy details in the rows, by design.

7. Withdraw / Settings

   - Withdraw amount input + confirm, active strategy list with

     view/expire actions.

Include clear loading and error states everywhere a wallet or circuit

call would happen (per Level 4 requirements: wallet connect, circuit

calls, loading states, error states are mandatory, not optional).

Use realistic placeholder data throughout. No backend logic yet — this

is the UI/UX scaffold, structured so real Lace wallet connection and

Midnight circuit calls can be wired into WalletConnect.tsx,

StrategyBuilder.tsx, and utils/contract.ts afterward without restructuring. wiith modern landing page and all those things not like seeing yeah this is ai like follow these prompt to built landing page        _ Build a React + Vite + Tailwind CSS landing page for "Axion Studio" - a design agency site. Use the `shaders` package (npm: `shaders`) for the hero background, `lucide-react` for icons. The page has 3 sections. Match every detail exactly:
...
Continue developing this project in the [Lovable editor](https://lovable.dev/projects/f98bf01f-6fe7-4437-924a-aea0b513ca08).
