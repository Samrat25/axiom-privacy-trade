...
async function setup() {
  ...
  if (!error) {
    console.log('✅ axiom_events table already exists and is accessible.');
    return;
  }
  ...
  console.log('');
  console.log(`CREATE TABLE IF NOT EXISTS axiom_events (
...
CREATE INDEX IF NOT EXISTS axiom_events_wallet_network_idx
  ON axiom_events (wallet_address, network);
CREATE INDEX IF NOT EXISTS axiom_events_network_status_idx
  ON axiom_events (network, status);`);
  console.log('');
