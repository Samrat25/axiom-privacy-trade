# Axiom — Full Product & Technical Documentation
...
Axiom proves the agent followed its committed strategy across its
...
cryptographic backing. Axiom sits in the gap between the two.

---

## 10. Naming

**Axiom** — a foundational statement accepted as true without needing
...
mapped to Axiom specifically. Only start Level 4 after the Level 3
...
axiom/
├── contracts/
│   └── axiom.compact              # commitStrategy, executeTrade, unshieldWithdraw
...
1. Write `contracts/axiom.compact` with public ledger state
...
6. Deploy `axiom.compact` to Preprod (manual step — get the exact
...
8. `README.md` in the mandated order: tagline, live demo link,
   **Contract Address table (mandatory)**, what it does, Privacy Model
   ...
   profile placeholder.
9. 3 launch tweets: what Axiom is and why it needs Midnight; a
